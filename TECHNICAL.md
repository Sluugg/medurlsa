...
| `logo_path` | Path to a logo image file for the LogoFlash overlay and favicon. Defaults to the committed `assets/logo.png` (ships with the repo, works with no `content_config.json` at all) rather than `None` — override it to use your own, e.g. something in `./branding` (bind-mounted, gitignored, meant for per-deployment customization). Under Docker, point this at `./branding`, not `./backgrounds` — `get_public_config()` (`app/routes/public_config.py`) excludes the logo's basename from `available_backgrounds` defensively, but a dedicated directory means there's no reason to rely on that exclusion in the first place |
